...
export default ({ app }, inject) => {
  ...
  const walkTokens = function(token){
    ...
    if (token.type === 'table'){   
      ...
      for (let iRow = 0; iRow < token.rows.length; iRow++){
        let row = token.rows[iRow];
        data[iRow] = [];

        for (let iCol = 0; iCol < row.length; iCol++){
          let col = row[iCol];

          if (col.text.trim().indexOf('=') === 0)
            formulaIndexs.push({ row: iRow, col: iCol });

          data[iRow].push(col.text);
        }
      }
        
      if (formulaIndexs.length > 0){
        const instance = HyperFormula.buildFromArray(data, {
          licenseKey: 'gpl-v3',
        });

        for (let formula of formulaIndexs){
          const { row, col } = formula;
          const colData = token.rows[row][col];

          colData.text = instance.getCellValue({ col, row, sheet: 0 }).toString();
          colData.tokens = this.Lexer.lexInline(colData.text);
        }
      }
    }
    ...
    if (token.type === 'table'){
      const formatCols = [];
      for (let iCol = 0; iCol < token.header.length; iCol++){
        if (token.header[iCol].text.indexOf('(VND)') !== -1)
          formatCols.push(iCol);
      }

      for (let row of token.rows){
        for (let iCol of formatCols){
          let text = new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' }).format(parseInt(row[iCol].text));
          row[iCol].tokens = this.Lexer.lexInline(text);
        }
      }
    }
  };
  ...
  const taskItem = {
    ...
    renderer(token) {
      const id = nanoid();
      const checkbox = `<input type="checkbox" id="ti-checkbox-${id}" ${token.checked ? 'checked ' : ''}onclick="return false;">`;
